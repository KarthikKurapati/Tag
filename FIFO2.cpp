...
Queue::Queue(){
  ...
  PutI = 0; //this is essentially the head
  GetI = 0; // this is essentially the tail
}
...
bool Queue::IsFull(void){
    if((PutI - 1 + FIFOSIZE) % FIFOSIZE == GetI){
      return true;
    }else{
      return false;
    }
...
bool Queue::Put(char x){
    ...
    Buf[PutI] = x;
  // decrement PutI and wrap around if below 0
...
void Queue::Print(void){
    ...
    // Step backward through the array to print
    while(current != PutI){
      ST7735_OutChar(Buf[current]);
      current = (current - 1 + FIFOSIZE) % FIFOSIZE;
    }
}

int Queue::Size(void){
    return (PutI - GetI + FIFOSIZE) % FIFOSIZE;
}
